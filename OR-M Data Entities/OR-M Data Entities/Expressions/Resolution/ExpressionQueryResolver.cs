...
namespace OR_M_Data_Entities.Expressions.Resolution
{
    ...
    {
        public static LambdaToSqlResolution Resolve<T>(IExpressionQuery<T> source, Expression<Func<T, bool>> expressionQuery)
        {
            return Resolve(source, new List<SqlDbParameter>(), expressionQuery);
        }

        public static LambdaToSqlResolution Resolve<T>(IExpressionQuery<T> source, List<SqlDbParameter> parameters, Expression<Func<T, bool>> expressionQuery)
        {
            return Resolve(source.Id, source.Tables, parameters, expressionQuery.Body);
        }

        public static LambdaToSqlResolution Resolve(Guid queryId, IQuerySchematic schematic, List<SqlDbParameter> parameters, Expression expressionQuery, bool isSubQuery = false)
        {
            QueryId = queryId;
        ...
        private static string _getSqlFromLambdaMethod(MemberExpression expression)
        {
            ...
            switch (methodName.ToUpper())
            {
                ...
                case "FIRSTORDEFAULT":
                    ...
                    var c = Resolve(QueryId, Schematic, Parameters, lambdaExpression.Body, true);
    ...
    public class ExpressionQuerySelectResolver : ExpressQueryResolverBase
    {
        ...
        public static LambdaToSqlResolution Resolve<TSource, TResult>(IExpressionQuery<TSource> source, Expression<Func<TSource, TResult>> expressionQuery)
        {
            return Resolve(source, new List<SqlDbParameter>(), expressionQuery);
        }

        public static LambdaToSqlResolution Resolve<TSource, TResult>(IExpressionQuery<TSource> source, List<SqlDbParameter> parameters, Expression<Func<TSource, TResult>> expressionQuery)
        {
            return Resolve(source.Id, source.Tables, parameters, expressionQuery.Body);
        }
