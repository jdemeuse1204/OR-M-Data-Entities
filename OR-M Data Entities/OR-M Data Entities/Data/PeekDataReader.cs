...
namespace OR_M_Data_Entities.Data
{
    ...
    public sealed class PeekDataReader : IDataReader
    {
        ...
        private bool _loadObject(object instance, string parentPropertyName)
        {
            try
            {
        ...
        private T _getObjectFromReaderWithForeignKeys<T>()
        {
            ...
            while (Peek() && compositeKey.Equals(compositeKeyArray.Sum(w => this[w].GetHashCode())) && Read())
            {
                _loadObjectWithForeignKeys(instance);
            }
        ...
        private long _getCompositKey(IDataLoadSchematic schematic)
        {
            return _getCompositKeyArray(schematic).Sum(w => this[w].GetHashCode());
        }
        ...
        private void _loadObjectWithForeignKeys(object startingInstance)
        {
            ...
            for (var i = 0; i < schematicsToScan.Count; i++)
            {
                ...
                if (!_loadObject(newInstance, currentSchematic.PropertyName)) continue;
        ...
        private void _loadObjectByColumnNames(object instance)
        {
            try
            {
                ...
                foreach (var property in properties)
                {
                    ...
                    var dbValue = this[columnAttribute == null ? property.Name : columnAttribute.Name];
